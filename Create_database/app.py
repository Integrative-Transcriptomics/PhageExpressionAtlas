...
def addFullDataset(phageName, phageDes, phageID, phageType,
               hostName, hostDes, hostID, hostGroup,
               rnaSeqPath, norm, name, journal, year, firstauthor, pubmedID, description, doi,
    ...
    upload_date: date of dataset integration in dataset
    """

    phageAdd = Phage(name = phageName, description = phageDes, ncbi_id = phageID, phage_type = phageType)
    hostAdd = Host(name = hostName, group = hostGroup, description = hostDes, ncbi_id = hostID)
    ...
    rnaSeq = Dataset(matrix_data = countsserial, normalization = norm, phage_id = phageAdd.id, host_id = hostAdd.id,
                     name = name, year = year, firstauthor = firstauthor, journal = journal,
                     pubmedID = pubmedID, description = description, doi = doi, upload_date = upload_date)
...
def addSingleDataset(rnaSeqPath, norm, name, journal, year, firstauthor, pubmedID, host_id, phage_id, description, doi, upload_date):
    countsTable = pd.read_csv(rnaSeqPath, index_col=0, sep='\t')
    countsserial = pickle.dumps(countsTable)
